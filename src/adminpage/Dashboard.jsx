...
const Dashboard = () => {
  ...
  return (
    <Layout
      ...
      >
      <Sider trigger={null} collapsible collapsed={collapsed} style={{ background: '#d4dadf', overflow: 'auto', height: '100vh', position: 'fixed', left: 0, top: 0, bottom: 0}}>
        <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full"><img src={logoWK} className="bi bi-person-fill text-white dark:text-indigo-300"alt="" /></div>        
        <Menu onClick={handleMenuClick} style={{background: 'none', color:'black'}}>
        ...
        <div className="mb-8 fixed flex justify-center bottom-0 uppercase" style={{marginLeft: isButtonClicked ? '14px' : '65px', transition: 'margin-left 0.3s ease-in-out'}}>
          <p className="tracking-tighter text-1xl font-black text-gray-900">Admin</p>
        </div>
      </Sider>
      <Layout
        style={{marginLeft: isButtonClicked ? '80px' : '200px', transition: 'margin-left 0.3s ease-in-out'}}>
        <Header
          style={{
            padding: 0,
