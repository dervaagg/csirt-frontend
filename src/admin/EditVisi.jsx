...
export default function EditLayananU() {
  ...
  const deleteVisi = async (visiId) => {
    try {
      await axios.delete(`http://localhost:4001/vission/${visiId}`);
      setVisi((prevVisi) =>
        prevVisi.filter((item) => item.id !== visiId)
      );
    } catch (error) {
      console.log(error);
    }
  ...
  const cancel = (e) => {
    console.log(e);
    message.error("Batal Menghapus Visi");
  };
